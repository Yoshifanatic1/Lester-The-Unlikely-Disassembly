...
set asarVer=asar
set ROMName=
set ROMName0=LTU_USA.sfc
set ROMName1=LTU_JAPAN.sfc
set MemMap=hirom
...
:Start
echo Place a headerless LTU ROM with one of the following names in this folder, then type the number representing what ROM to extract from.
echo 0 = "%ROMName0%" (LTU USA)
echo 1 = "%ROMName1%" (LTU Japan)
...
if %PointerSet% equ 24 goto :MoveLevel
...
echo:^^!LoopCounter #= 0 >> TEMP1.asm
echo:dd snestopc(DataPointers) : dw ^^!FilesToCombine : dw ^^!FileType : db "LTU     " : db "RDC V1.0.0" >> TEMP1.asm
echo:db $00,$00,$00,$00,$00,$00 >> TEMP1.asm
echo:db "Lester the Unlikely Level Data File" >> TEMP1.asm
